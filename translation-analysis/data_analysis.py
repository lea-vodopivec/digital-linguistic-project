...
def identify_red_cells(df):
    """
    Identify potential "red cells" where idiom is correct but sentence may be wrong.
    Based on pattern: meaning='yes' with type=4 (other) might indicate this.
    Also flagging any anomalies for manual review.
    """
    # Red cells are where idiom is translated correctly (meaning=yes) 
    # but something is unusual - we'll identify by looking at type=4 with yes
    df['potential_red_cell'] = (df['meaning'] == 'yes') & (df['type'] == 4)
    return df
...
def calculate_red_cell_stats(df):
    """Calculate Layer 3 statistics: Red cells analysis."""
    stats = {}
    
    # Count potential red cells per system
    red_counts = df[df['potential_red_cell']].groupby('system').size()
    stats['red_cell_counts'] = red_counts.to_dict()
    
    # List of red cell cases
    red_cases = df[df['potential_red_cell']][['idiom', 'sentence_num', 'system', 'meaning', 'type']]
    stats['red_cell_cases'] = red_cases
...
def generate_report(df, layer1_stats, layer2_stats, combination_stats, red_cell_stats, output_dir):
    ...
    # Red Cells Analysis
    report.append("\n---\n## Layer 3: Special Cases (Potential Red Cells)\n")
    report.append("*Cases where idiom may be correct but sentence context is unusual (Type 4 with meaning=yes)*\n")
    
    report.append("### Count by System\n")
    report.append("| System | Count |")
    report.append("|--------|-------|")
    for system in ALL_SYSTEMS:
        count = red_cell_stats['red_cell_counts'].get(system, 0)
        report.append("| {} | {} |".format(system, count))
...
def main():
    ...
    # Identify red cells
    print("\n[2/6] Identifying potential red cells...")
    df = identify_red_cells(df)
    red_count = df['potential_red_cell'].sum()
    print("    Found {} potential red cell cases".format(red_count))
    
    # Calculate statistics
    print("\n[3/6] Calculating Layer 1 statistics (Meaning)...")
    layer1_stats = calculate_layer1_stats(df)
    
    print("\n[4/6] Calculating Layer 2 statistics (Type)...")
    layer2_stats = calculate_layer2_stats(df)
    
    print("\n[5/6] Calculating combination and red cell statistics...")
    combination_stats = calculate_combination_stats(df)
    red_cell_stats = calculate_red_cell_stats(df)
    
    # Generate outputs
    print("\n[6/6] Generating visualizations and report...")
    create_visualizations(df, layer1_stats, layer2_stats, combination_stats, output_dir)
    generate_report(df, layer1_stats, layer2_stats, combination_stats, red_cell_stats, output_dir)
